...
AdditionalCarbonAccountability <-
  ...
  foreach(AllocationPrincipleCB = c("Carbon debt and Equality", "Equal cumulative per capita", "Grandfathering", "Capability", "Contraction and convergence"), .combine = "rbind") %:%
  foreach(AllocationPrincipleEAP = c("Carbon debt and Equality", "Equal cumulative per capita", "Grandfathering", "Capability", "Contraction and convergence"), .combine = "rbind") %:%
  foreach(CarbonDebtAssumption = c(1990, 1960, 1980, 2000, FALSE), .combine = "rbind") %dopar% {
    ...
    for (Country in CountryAssumptions$Country) {
      ## STEP 1 ##
      CarbonDebt = if (CarbonDebtAssumption != FALSE & AllocationPrincipleCB != "Equal cumulative per capita") { #Equation 1
      ...
      NationalFutureCarbonBudget = 
        if (AllocationPrincipleCB == "Carbon debt and Equality") { #Estimating the second term of Equation 2
          ...
          CarbonBudget$BudgetGtCO2[CarbonBudget$TempTarget == TempTarget] * 1000 * 
            DataGlobalCarbonBudget$EmissionsMtCO2[DataGlobalCarbonBudget$Country == Country & DataGlobalCarbonBudget$Year == 2022] /
            DataGlobalCarbonBudget$EmissionsMtCO2[DataGlobalCarbonBudget$Country == "World" & DataGlobalCarbonBudget$Year == 2022]
        } else if (AllocationPrincipleCB == "Equal cumulative per capita") { #Estimating the second term of Equation 2 based on Supplementary Equation 3
    ...
    while (EmissionAllowancesPool > 0) { # Iteratively solve Equation 7
      ...
      for (Country in CountryAssumptions$Country) {
        if (Country %in% AdditionalCarbonAccountabilityIteration$Country[AdditionalCarbonAccountabilityIteration$Iteration == n-1 & AdditionalCarbonAccountabilityIteration$TotalExcessiveCarbonClaims > 0 & AdditionalCarbonAccountabilityIteration$Country != "World"]) {
          if (AllocationPrincipleEAP %in% c("Carbon debt and Equality", "Equal cumulative per capita")) { # Allocating the EAP based on Equation 6 and Supplementary Equation 4
...
TableForManuscript <- subset(AdditionalCarbonAccountability, AllocationPrincipleCB == "Carbon debt and Equality" & AllocationPrincipleEAP == "Carbon debt and Equality" & CarbonDebtAssumption == 1990 & !Country %in% c("World" ,"Rest of world")) # Subset the data for the table
...
TableForManuscript <- rbind(TableForManuscript, subset(AdditionalCarbonAccountability, AllocationPrincipleCB == "Carbon debt and Equality" & AllocationPrincipleEAP == "Carbon debt and Equality" & CarbonDebtAssumption == 1990 & Country %in% c("World" ,"Rest of world"))) # Add the data for the rest of the world
...
DataForZoomFigure <- subset(AdditionalCarbonAccountability, AllocationPrincipleCB == "Carbon debt and Equality" & AllocationPrincipleEAP == "Carbon debt and Equality" & Country != "World" & CarbonDebtAssumption == 1990) # Subset the data for the zoom figure
...
for (iCB in unique(DataFiguresSensitivity$AllocationPrincipleCB)) {
  for (iEAP in unique(DataFiguresSensitivity$AllocationPrincipleEAP)) {
    for (j in unique(DataFiguresSensitivity$CarbonDebtAssumption)) {
      DataFiguresSensitivity$ChangeComparedToMain[DataFiguresSensitivity$AllocationPrincipleCB == iCB & DataFiguresSensitivity$AllocationPrincipleEAP == iEAP & DataFiguresSensitivity$CarbonDebtAssumption == j] <- 100*(DataFiguresSensitivity$AdditionalCarbonAccountability[DataFiguresSensitivity$AllocationPrincipleCB == iCB & DataFiguresSensitivity$AllocationPrincipleEAP == iEAP & DataFiguresSensitivity$CarbonDebtAssumption == j] / DataFiguresSensitivity$AdditionalCarbonAccountability[DataFiguresSensitivity$AllocationPrincipleCB == "Carbon debt and Equality" & DataFiguresSensitivity$AllocationPrincipleEAP == "Carbon debt and Equality"  & DataFiguresSensitivity$CarbonDebtAssumption == 1990] - 1)
      DataFiguresSensitivity$ChangeComparedToMainPerCapita[DataFiguresSensitivity$AllocationPrincipleCB == iCB & DataFiguresSensitivity$AllocationPrincipleEAP == iEAP & DataFiguresSensitivity$CarbonDebtAssumption == j] <- 100*(DataFiguresSensitivity$AdditionalCarbonAccountabilityPerCapita[DataFiguresSensitivity$AllocationPrincipleCB == iCB & DataFiguresSensitivity$AllocationPrincipleEAP == iEAP & DataFiguresSensitivity$CarbonDebtAssumption == j] / DataFiguresSensitivity$AdditionalCarbonAccountabilityPerCapita[DataFiguresSensitivity$AllocationPrincipleCB == "Carbon debt and Equality" & DataFiguresSensitivity$AllocationPrincipleEAP == "Carbon debt and Equality" & DataFiguresSensitivity$CarbonDebtAssumption == 1990] - 1)
      DataFiguresSensitivity$TotExcessiveChangeToMain[DataFiguresSensitivity$AllocationPrincipleCB == iCB & DataFiguresSensitivity$AllocationPrincipleEAP == iEAP & DataFiguresSensitivity$CarbonDebtAssumption == j] <- 100*(DataFiguresSensitivity$TotalExcessiveCarbonClaims[DataFiguresSensitivity$AllocationPrincipleCB == iCB & DataFiguresSensitivity$AllocationPrincipleEAP == iEAP & DataFiguresSensitivity$CarbonDebtAssumption == j] / DataFiguresSensitivity$TotalExcessiveCarbonClaims[DataFiguresSensitivity$AllocationPrincipleCB == "Carbon debt and Equality" & DataFiguresSensitivity$AllocationPrincipleEAP == "Carbon debt and Equality" & DataFiguresSensitivity$CarbonDebtAssumption == 1990] - 1)
    }
  }
}
...
SensitivityAnalysisCarbonDebtAssumption <- ggplot() +
  geom_col(data = subset(DataFiguresSensitivity, !Country %in% c("World", "Rest of world") & TempTarget == 1.5 & (AllocationPrincipleCB == "Carbon debt and Equality" & AllocationPrincipleEAP == "Carbon debt and Equality") & Graph != factor("ChangeComparedToMain", levels = c("AdditionalCarbonAccountability", "AdditionalCarbonAccountabilityPerCapita", "ChangeComparedToMain"), 
  ...
  geom_rect(data = subset(DataFiguresSensitivity, !Country %in% c("World", "Rest of world") & TempTarget == 1.5 & (AllocationPrincipleCB == "Carbon debt and Equality" & AllocationPrincipleEAP == "Carbon debt and Equality") & CarbonDebtAssumption != 1990 & Graph == factor("ChangeComparedToMain", levels = c("AdditionalCarbonAccountability", "AdditionalCarbonAccountabilityPerCapita", "ChangeComparedToMain"), 
                                                                                                                                                                                                                                                       labels = c(expression(paste("Additional carbon accountability (Gt", CO[2],")")), expression(paste("Additional carbon accountability per capita (t", CO[2],")")), expression(paste("Change compared to main case (%)"))))),
  ...
  geom_text(data = rbind(data.frame(x = 225, y = 1, label = "a)", Graph = factor("AdditionalCarbonAccountability", levels = c("AdditionalCarbonAccountability", "AdditionalCarbonAccountabilityPerCapita", "ChangeComparedToMain"), 
                         ...
                         data.frame(x = 4, y = 1, label = "c)", Graph = factor("ChangeComparedToMain", levels = c("AdditionalCarbonAccountability", "AdditionalCarbonAccountabilityPerCapita", "ChangeComparedToMain"), 
                                                                                labels = c(expression(paste("Additional carbon accountability (Gt", CO[2],")")), expression(paste("Additional carbon accountability per capita (t", CO[2],")")), expression(paste("Change compared to main case (%)")))))),
            mapping = aes(x = x, y = y, label = label)) +
  geom_vline(data = subset(DataFiguresSensitivity, Country == "World" & TempTarget == 1.5 & CarbonDebtAssumption == 1990 & AllocationPrincipleCB == "Carbon debt and Equality" & AllocationPrincipleEAP == "Carbon debt and Equality" & Graph == factor("AdditionalCarbonAccountabilityPerCapita", levels = c("AdditionalCarbonAccountability", "AdditionalCarbonAccountabilityPerCapita", "ChangeComparedToMain"), 
                                                                                                                                                                                               labels = c(expression(paste("Additional carbon accountability (Gt", CO[2],")")), expression(paste("Additional carbon accountability per capita (t", CO[2],")")), expression(paste("Change compared to main case (%)"))))),
             mapping = aes(xintercept = Change), linetype = "dashed") +
  geom_text(size = 2.5, data = subset(DataFiguresSensitivity, Country == "World" & TempTarget == 1.5 & CarbonDebtAssumption == 1990 & AllocationPrincipleCB == "Carbon debt and Equality" & Graph == factor("AdditionalCarbonAccountabilityPerCapita", levels = c("AdditionalCarbonAccountability", "AdditionalCarbonAccountabilityPerCapita", "ChangeComparedToMain"), 
                                                                                                                                                                                                          labels = c(expression(paste("Additional carbon accountability (Gt", CO[2],")")), expression(paste("Additional carbon accountability per capita (t", CO[2],")")), expression(paste("Change compared to main case (%)"))))),
...
SensitivityAnalysisAllocationPrincipleOnlyCB <- ggplot() +
  geom_col(data = subset(DataFiguresSensitivity, !Country %in% c("World", "Rest of world") & TempTarget == 1.5 & AllocationPrincipleEAP == "Carbon debt and Equality" & CarbonDebtAssumption == 1990 & Graph != factor("ChangeComparedToMain", levels = c("AdditionalCarbonAccountability", "AdditionalCarbonAccountabilityPerCapita", "ChangeComparedToMain"), 
                                                                                                                                                                                                                                                                                                                                                                   labels = c(expression(paste("Additional carbon accountability (Gt", CO[2],")")), expression(paste("Additional carbon accountability per capita (t", CO[2],")")), expression(paste("Change compared to main case (%)"))))),
           mapping = aes(x = ifelse(Change == Inf, NA, Change), y = Country, fill = AllocationPrincipleCB), position = "dodge") +
  geom_rect(data = subset(DataFiguresSensitivity, !Country %in% c("World", "Rest of world") & TempTarget == 1.5 & AllocationPrincipleEAP == "Carbon debt and Equality" & CarbonDebtAssumption == 1990 & Graph == factor("ChangeComparedToMain", levels = c("AdditionalCarbonAccountability", "AdditionalCarbonAccountabilityPerCapita", "ChangeComparedToMain"), 
                                                                                                                                                                                                                                                                                                                                                                    labels = c(expression(paste("Additional carbon accountability (Gt", CO[2],")")), expression(paste("Additional carbon accountability per capita (t", CO[2],")")), expression(paste("Change compared to main case (%)"))))),
            mapping = aes(xmin = -Inf, xmax = Inf, ymin = -Inf, ymax = Inf), fill = "white") +
  geom_text(size = 3, data = subset(DataFiguresSensitivity, !Country %in% c("World", "Rest of world") & TempTarget == 1.5 & AllocationPrincipleEAP == "Carbon debt and Equality" & AllocationPrincipleCB != "Carbon debt and Equality" & CarbonDebtAssumption == 1990 & Graph == factor("ChangeComparedToMain", levels = c("AdditionalCarbonAccountability", "AdditionalCarbonAccountabilityPerCapita", "ChangeComparedToMain"), 
                                                                                                                                                                                                                                                                                                                                                                                                                                    labels = c(expression(paste("Additional carbon accountability (Gt", CO[2],")")), expression(paste("Additional carbon accountability per capita (t", CO[2],")")), expression(paste("Change compared to main case (%)"))))),
            mapping = aes(x = ifelse(AllocationPrincipleCB == "Grandfathering", 0.5, 0) +
                            ifelse(AllocationPrincipleCB == "Equal cumulative per capita", 1.5, 0) +
  ...
  geom_text(data = rbind(data.frame(x = 200, y = 1, label = "a)", Graph = factor("AdditionalCarbonAccountability", levels = c("AdditionalCarbonAccountability", "AdditionalCarbonAccountabilityPerCapita", "ChangeComparedToMain"), 
                         ...
                         data.frame(x = 4, y = 1, label = "c)", Graph = factor("ChangeComparedToMain", levels = c("AdditionalCarbonAccountability", "AdditionalCarbonAccountabilityPerCapita", "ChangeComparedToMain"), 
                                                                               labels = c(expression(paste("Additional carbon accountability (Gt", CO[2],")")), expression(paste("Additional carbon accountability per capita (t", CO[2],")")), expression(paste("Change compared to main case (%)")))))),
            mapping = aes(x = x, y = y, label = label)) +
  geom_vline(data = subset(DataFiguresSensitivity, Country == "World" & TempTarget == 1.5 & CarbonDebtAssumption == 1990 & AllocationPrincipleEAP == "Carbon debt and Equality" & AllocationPrincipleCB == "Carbon debt and Equality" & Graph == factor("AdditionalCarbonAccountabilityPerCapita", levels = c("AdditionalCarbonAccountability", "AdditionalCarbonAccountabilityPerCapita", "ChangeComparedToMain"), 
                                                                                                                                                                                                                                                   labels = c(expression(paste("Additional carbon accountability (Gt", CO[2],")")), expression(paste("Additional carbon accountability per capita (t", CO[2],")")), expression(paste("Change compared to main case (%)"))))),
             mapping = aes(xintercept = Change), linetype = "dashed") +
  geom_text(size = 2.5, data = subset(DataFiguresSensitivity, Country == "World" & TempTarget == 1.5 & CarbonDebtAssumption == 1990 & AllocationPrincipleEAP == "Carbon debt and Equality" & AllocationPrincipleCB == "Carbon debt and Equality" & Graph == factor("AdditionalCarbonAccountabilityPerCapita", levels = c("AdditionalCarbonAccountability", "AdditionalCarbonAccountabilityPerCapita", "ChangeComparedToMain"), 
                                                                                                                                                                                                                                                              labels = c(expression(paste("Additional carbon accountability (Gt", CO[2],")")), expression(paste("Additional carbon accountability per capita (t", CO[2],")")), expression(paste("Change compared to main case (%)"))))),
...
SensitivityAnalysisAllocationPrincipleOnlyEAP <- ggplot() +
  geom_col(data = subset(DataFiguresSensitivity, !Country %in% c("World", "Rest of world") & TempTarget == 1.5 & AllocationPrincipleCB == "Carbon debt and Equality" & CarbonDebtAssumption == 1990 & Graph != factor("ChangeComparedToMain", levels = c("AdditionalCarbonAccountability", "AdditionalCarbonAccountabilityPerCapita", "ChangeComparedToMain"), 
                                                                                                                                                                                                                                                                                                                                                                                labels = c(expression(paste("Additional carbon accountability (Gt", CO[2],")")), expression(paste("Additional carbon accountability per capita (t", CO[2],")")), expression(paste("Change compared to main case (%)"))))),
           mapping = aes(x = ifelse(Change == Inf, NA, Change), y = Country, fill = AllocationPrincipleEAP), position = "dodge") +
  geom_rect(data = subset(DataFiguresSensitivity, !Country %in% c("World", "Rest of world") & TempTarget == 1.5 & AllocationPrincipleCB == "Carbon debt and Equality" & CarbonDebtAssumption == 1990 & Graph == factor("ChangeComparedToMain", levels = c("AdditionalCarbonAccountability", "AdditionalCarbonAccountabilityPerCapita", "ChangeComparedToMain"), 
                                                                                                                                                                                                                                                                                                                                                                                labels = c(expression(paste("Additional carbon accountability (Gt", CO[2],")")), expression(paste("Additional carbon accountability per capita (t", CO[2],")")), expression(paste("Change compared to main case (%)"))))),
            mapping = aes(xmin = -Inf, xmax = Inf, ymin = -Inf, ymax = Inf), fill = "white") +
  geom_text(size = 3, data = subset(DataFiguresSensitivity, !Country %in% c("World", "Rest of world") & TempTarget == 1.5 & AllocationPrincipleCB == "Carbon debt and Equality" & AllocationPrincipleEAP != "Carbon debt and Equality" & CarbonDebtAssumption == 1990 & Graph == factor("ChangeComparedToMain", levels = c("AdditionalCarbonAccountability", "AdditionalCarbonAccountabilityPerCapita", "ChangeComparedToMain"), 
                                                                                                                                                                                                                                                                                                                                                                                                                                                  labels = c(expression(paste("Additional carbon accountability (Gt", CO[2],")")), expression(paste("Additional carbon accountability per capita (t", CO[2],")")), expression(paste("Change compared to main case (%)"))))),
            mapping = aes(x = ifelse(AllocationPrincipleEAP == "Grandfathering", .5, 0) +
                            ifelse(AllocationPrincipleEAP == "Equal cumulative per capita", 1.5, 0) +
